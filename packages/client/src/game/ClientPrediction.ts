...
export class ClientPrediction {
  ...
  predictPosition(currentPosition: THREE.Vector3, input: PlayerInput, deltaTime: number): Vector3 {
  ...
  reconcile(serverPlayer: Player, serverFrameId: number): void {
    ...
    const relevantInputs = this.inputHistory.filter(history => history.frameId > serverFrameId);
